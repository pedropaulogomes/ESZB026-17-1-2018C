...
void enviaDadosPelaSerial(){              // opcional, para debug
  ...
  Serial.print(buffer[5],HEX); Serial.print("."); // valor mais significativo (0<=A<=5)
  Serial.println(buffer[6],HEX); // valor menos significativo (0<=B<=99)
}
...
void loop(){                       // Le o valor no potenciometro a cada 1 segundo
  ...
  byte A = int(valorRecebido*(5.0f/1023.0f));
  byte B = (int)((valorRecebido*(5.0f/1023.0f) - (float)A)*100.0f);
  buffer[5] = A;
  buffer[6] = B;
